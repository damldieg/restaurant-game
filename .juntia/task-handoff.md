<!-- juntia:generated -->
<!-- juntia:task-meta {"text":"Agregar la fórmula real de deriveSpawnIntervalMs basada en reputación y reemplazar el spawn fijo de CustomerSystem por ella","generatedAt":"2026-08-22T17:00:06.229Z"} -->
# Task Handoff
...
> Agregar la fórmula real de deriveSpawnIntervalMs basada en reputación y reemplazar el spawn fijo de CustomerSystem por ella
...
- Q: "A partir de M07, ¿qué criterio prioriza la planificación de milestones: features jugables con cambio observable, o preparación arquitectónica/documentación previa a la feature real?" -> CONFIRMED: Priorizar features jugables con cambio observable para el jugador. Documentación/refactor arquitectónico solo dentro del milestone de gameplay que lo necesita (no como milestone independiente), salvo que evite una refactorización costosa o defina una responsabilidad nueva importante. M07.1 (ya completado como contrato de tipos, PR #32) queda como historial válido bajo el criterio anterior; M07.2 en adelante sigue el nuevo criterio. [corrected: the original CLI submission recorded only "1", a stdin transcription error, not the human's real answer — corrected inline per DECISIONS.md's own correction rule] (product decision, 2026-08-22)
  (options on the table when asked: Priorizar features jugables: cada milestone debe producir un cambio observable para el jugador; documentar/refactorizar solo dentro del milestone de gameplay que lo necesita, nunca como milestone independiente salvo que evite una refactorización costosa o defina una responsabilidad nueva importante., Mantener el criterio actual (M00-M06): pasos de foundation/documentación explícitos antes de cada feature real, incluso sin cambio visible.)
- Q: "¿Qué valores de balance (mínimo/máximo de intervalo de spawn, y puntos de referencia de reputación) debe usar deriveSpawnIntervalMs(reputation) en M07.2?" -> CONFIRMED: Moderado: intervalo 1200ms (reputacion >= 15) a 5000ms (reputacion <= -5), interpolacion lineal entre esos puntos. En reputacion=0 (inicio de partida) el intervalo es ~3600ms, cercano al baseline actual de 2500ms. (product decision, 2026-08-22)
  (options on the table when asked: Moderado: intervalo 1200ms (rep >= +15) a 5000ms (rep <= -5), interpolación lineal entre esos puntos. En rep=0 (inicio de partida) el intervalo es ~3600ms, cercano al baseline actual de 2500ms., Ajustado (tight): intervalo 1500ms a 4000ms sobre el mismo rango de reputación (-5..+15) — la demanda varía menos dramáticamente con la reputación., Amplio (wide): intervalo 1000ms a 6000ms sobre el mismo rango de reputación (-5..+15) — un restaurante con mala reputación se siente notablemente vacío, uno con buena reputación notablemente ocupado.)
