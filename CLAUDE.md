...
- Keep `docs/PROJECT_STATE.md` concise and factual. Update it only at a completed milestone, a material decision, or when `/checkpoint` is requested. Do not update it after every small task.
...
| Level | Agent | Use for |
| --- | --- | --- |
| tiny | `maintenance-tiny` | State updates, narrow checks, short diffs, file discovery, simple documentation. |
| normal | `feature-normal` | A bounded gameplay, UI, content, or integration feature. |
| deep | `systems-deep` | Reproducible difficult bugs, performance issues, cross-cutting systems, or risky refactors. |
| architect | `architecture-review` | Compare consequential architectural options before committing to one. |
...
A milestone is a user-visible feature slice, a completed bug investigation/fix, a validated system change, a release/commit-ready checkpoint, or a decision that changes future implementation. At a milestone, run `/checkpoint` (or make its equivalent update) once.
